...
class SecureClient(object):
    ...
    def _make_socket(self, socktype):
        ...
        return SecureSocket.decorate(socket, self._server_key.public, socktype)
...
class SecureServer(object):
    ...
    def _make_socket(self, socktype):
        ...
        return SecureSocket.decorate(socket, self._key.public, socktype)


class SecureSocket(object):

    def __init__(self, socket, server_publickey, socktype):
        self._socket = socket
        self._server_publickey = server_publickey
        self._orig_send_multipart = socket.send_multipart
        self._orig_recv_multipart = socket.recv_multipart
        self._socktype = socktype
        assert self._socktype in (zmq.ROUTER, zmq.DEALER, zmq.REQ), \
            "Can't secure socktype {} yet".format(socktype)

    def recv_multipart(self):
        msg = self._orig_recv_multipart()
        if self._socktype == zmq.ROUTER:
            # [addr, '', key, ...]
            assert msg[1] == ''
            assert msg[2] == self._server_publickey
            msg = [msg[0]] + msg[3:]
        elif self._socktype == zmq.REQ:
            # [key, ...]
            assert msg[0] == self._server_publickey
            assert msg[1] == ''
            msg = msg[2:]
        elif self._socktype == zmq.DEALER:
            # ['', key, ...]
            assert msg[0] == ''
            assert msg[1] == self._server_publickey
            msg = msg[2:]
        return msg

    def send_multipart(self, msg):
        if self._socktype == zmq.ROUTER:
            # Socket will strip msg[0] (the address)
            msg = [msg[0]] + ['', self._server_publickey] + msg[1:]
        elif self._socktype == zmq.REQ:
            # Socket will add an empty frame
            msg = [self._server_publickey, ''] + msg
        elif self._socktype == zmq.DEALER:
            # Socket does nothing
            msg = ['', self._server_publickey] + msg
        self._orig_send_multipart(msg)

    @classmethod
    def decorate(cls, socket, *args, **kwargs):
        ss = cls(socket, *args, **kwargs)
        socket.send_multipart = ss.send_multipart
        socket.recv_multipart = ss.recv_multipart
        return socket
